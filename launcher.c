/* Menu-bar launcher for build/ChatterBot.app.
 *
 * Runs the chatterbot package in-process via libpython (py2app-style)
...
int main(int argc, char **argv) {
     ...
     *   dev:      <root>/build/ChatterBot.app/Contents/MacOS/ChatterBot  → <root>
     *   homebrew: <cellar>/Applications/ChatterBot.app/Contents/MacOS/ChatterBot → <cellar> */
    for (int i = 0; i < 5; i++) {
    ...
    if (access(site, F_OK) == 0) {
        ...
        if (access(site, F_OK) != 0) {
            system("osascript -e 'display alert \"ChatterBot\" message "
     ...
     *      include root so the chatterbot package directory is importable directly. */
    ...
    char *py_argv[] = {
        argv[0],
        "-c",
        "from chatterbot.__main__ import main; main()",
        NULL,
    };
